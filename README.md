...
- 5종 한국어 프롬프트 생성
  ...
  - 시간복잡도 평가
  - 실행 오류 분석
- 사용자가 편집할 수 있는 프롬프트 미리보기와 클립보드 복사
- 기본 ChatGPT 웹 열기
...
5. `생성된 프롬프트 복사`를 누릅니다. 이 버튼을 누를 때만 문제와 코드가 클립보드에 기록됩니다.
6. `ChatGPT 열기`를 누릅니다.
7. ChatGPT 입력창에 직접 `Ctrl+V`하고 직접 전송합니다.
...
| 권한 | 이유 |
| --- | --- |
| `sidePanel` | Chrome 사이드 패널 UI와 툴바 아이콘 열기 동작을 사용합니다. |
...
3. 상태가 `일반 연습문제`, 언어가 `Python3`인지 확인합니다.
4. 목록 페이지나 pathname에 `assessment`, `exam`, `test`, `skill-check`, `coding-test`가 들어간 페이지에서는 정확한 미지원 메시지가 나오는지 확인합니다.
